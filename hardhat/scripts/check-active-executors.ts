/**
 * Discover active executors in TEEServiceRegistry across all capabilities.
 */
...
async function main() {
  ...
  console.log("ACTIVE EXECUTOR DISCOVERY");
  console.log("═".repeat(64));

  for (let cap = 0; cap < 10; cap++) {
    const raw = await client.call({
      to: TEE_REGISTRY,
      data: buildCalldata(cap, true), // Only active executors!
    });
    ...
    console.log(`Capability ${cap} has ${arrayLength} ACTIVE executors:`);
